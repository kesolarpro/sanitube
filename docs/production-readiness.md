...
| The *submission* path uses that description | NOT_READY | `Distributor::submitRelease()` takes the `Release` aggregate, not `ReleasePackage` — so a real adapter would walk tracks, credits and identifiers itself, which is the failure that type was created to prevent. Nothing is broken today: the only adapters are `none` and a fake. Changing the contract is cheap now and expensive after the first real adapter, so it is named rather than left to be discovered. |
...
| Index screens this test does not reach | NOT_READY | PERF-002 names five — enrichment, distribution, production, generations, projects — with what each would need. Each requires a chain of domain objects the catalogue fixture does not build. PERF-004 read all five and found one real N+1, now fixed and held where its objects live rather than in the scale fixture. The other four are eager-loaded. |
