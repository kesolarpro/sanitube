...
| How much is being stored is on a screen | READY | STO-005. `byte_size` has been on every asset since the first upload and the question was asked nowhere, so an operator could not find out how much they were keeping without a database client — which on a metered object store is the number that becomes a bill. The dashboard reported an asset *count*, which says nothing about it: a thousand previews and a thousand masters are the same number and three orders of magnitude apart. |
| The figure says whose number it is | READY | STO-005. What the catalogue records having stored, never what a provider bills, and the panel says so first. The two disagree for real reasons — an abandoned multipart upload leaves parts nothing registered, a bucket may hold objects from before this installation, versioning keeps what a delete removed — so calling it "your storage usage" would offer a number somebody reconciles against an invoice and cannot. |
| No capacity, and no percentage | READY | STO-005. A percentage needs a denominator this platform cannot honestly obtain: an object store publishes no quota it can read, a plan's limit is on the provider's own dashboard, and inventing one from a free tier would be a guess with a progress bar drawn around it. Free space on a local volume is a different question, knowable, and the doctor already answers it. |
| Trashed bytes are counted and shown apart | READY | STO-005. They still cost, which is the entire reason to show them: an operator watching a total that will not go down needs to know how much of it is waiting on a decision they have not made. TRASH-001 kept those objects on purpose; this is where that decision becomes visible rather than merely correct. |
| Unmeasured is never reported as zero | READY | STO-005. An installation storing nothing and one whose assets table would not answer are different answers, and reporting the second as the first would tell an operator their bill should be zero. |
...
| A plan can be made from the product | READY | PROD-002. `WriteProductionPlan::create` shipped with PROD-001 and had no caller anywhere: no controller, no console command, no seeder. Neither did `WriteEditorialProfile`, which a plan requires. So a working installation had a production screen that could only ever be empty, and the one part of SaniTube that acts unattended could be started only from a database client. |
| A plan's terms can be corrected | READY | PROD-002. Name, imprint, cadence, target and notes, and nothing else. The status is absent by design: pausing, resuming and disabling are named acts, and `EXHAUSTED` is a conclusion the platform draws from its own counting rather than a state a form assigns. The slug is frozen at creation, because it is how a console command names a plan and one that followed a rename would orphan every reference. |
| An imprint can be made and retired | READY | PROD-002. Everything about a particular label lives in these rows and never in code, so a second imprint is a second row — but nothing in the product could make the first. Retiring rather than deleting: every plan pointed at an imprint refers to it under a foreign key that refuses the deletion, so a delete button would work only for imprints nothing has ever used. |
| A plan is never pointed at a retired imprint | READY | PROD-002. Refused on creation and on correction, and the screen offers active ones only — a choice that fails on save is worse than a choice not offered. |
| The terms a plan spends on are in the log | READY | PROD-002. Cadence and target travel in the audit context on both create and correct, with the previous pair beside the new one: raising a cadence is what makes a plan spend more, and a plan's row holds only what it is now. The imprint's own guidance never does — that is the label's writing, and an audit log is not where a copy of it accumulates one revision per edit. |
