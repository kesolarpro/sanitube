...
/**
 * ART-004. Asking for a cover.
 *
 * The button exists only because the ceiling does. ART-003 measured whether
 * generation was possible here and said so; it stopped short of a trigger
 * because a paid call had no bound.
 *
 * Nothing is generated by this request: the record is written, the work is
 * queued, and the list below shows what has been asked for. A generated cover
 * becomes an ordinary Asset and still has to be **chosen** — generating is not
 * selecting.
 */
const coverPrompt = ref('');
const generating = ref(false);

const artworkRefusal = computed(() => {
    const errors = page.props.errors as Record<string, string> | undefined;

    return errors?.artwork ?? null;
});

/** Windows with a ceiling set. One without is not a limit to display. */
const remainingWindows = computed(() =>
    Object.entries(props.release.artwork_generation.remaining).filter(
...
function requestCover(): void {
    ...
    router.post(
        `/releases/${props.release.uuid}/cover/generate`,
...
<template>
    <div class="space-y-4">
        ...
        <AppCard>
            ...
            <!-- ART-004. The trigger, which exists only because the ceiling
                 does. A refused request is shown by its code; the server never
                 sends a sentence. -->
            <AppAlert
                v-if="artworkRefusal !== null"
                tone="danger"
                class="mt-3"
                :title="trans(`ui.releases.artwork_refusal.${artworkRefusal}`)"
            />

            <form
                v-if="release.artwork_generation.available && release.artwork_generation.can_request"
                class="mt-4 space-y-3"
                @submit.prevent="requestCover"
            >
                <FormField
                    :label="trans('ui.releases.cover_prompt')"
                    :hint="trans('ui.releases.cover_prompt_hint')"
                >
                    <TextareaInput v-model="coverPrompt" :rows="3" />
                </FormField>

                <div class="flex flex-wrap items-center justify-between gap-2">
                    <!-- What is left. A ceiling nobody can see is a ceiling
                         somebody hits by surprise. -->
                    <p v-if="remainingWindows.length > 0" class="text-caption text-muted">
                        {{
                            remainingWindows
                                .map(([window, left]) =>
                                    trans('ui.releases.cover_remaining', {
                                        window: trans(`ui.releases.cover_window.${window}`),
                                        left: String(left),
                                    }),
                                )
                                .join(' · ')
                    ...
                    <AppButton
                        ...
                        variant="secondary"
                        :loading="generating"
                        :disabled="coverPrompt.trim().length < 8"
                    >
                        {{ trans('ui.releases.cover_generate') }}
                    </AppButton>
                </div>
            </form>

            <!-- What has been asked for. Generating is not choosing: a finished
                 request produces an asset that still has to be selected as the
                 cover, through the picker above. -->
            <div v-if="release.artwork_generation.requests.length > 0" class="mt-4 space-y-2">
                <p class="text-caption text-muted">{{ trans('ui.releases.cover_history') }}</p>
                <ul class="divide-y divide-border">
                    <li
                        v-for="request in release.artwork_generation.requests"
                        :key="request.uuid"
                        class="flex flex-col gap-1 py-2"
                    >
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <span class="truncate text-small text-foreground">{{ request.prompt }}</span>
                            <StatusBadge group="artwork" :status="request.status" />
                        </div>
                        <p v-if="request.failure !== null" class="text-small text-danger">
                            {{ trans(`ui.releases.artwork_refusal.${request.failure}`) }}
                        </p>
                        <p v-else-if="request.model !== null" class="text-caption text-muted">
                            {{ trans('ui.releases.cover_made_by', { provider: request.provider, model: request.model }) }}
                        </p>
                    </li>
                </ul>
            </div>
